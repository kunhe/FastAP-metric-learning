function [net,stats] = train_hard_vid(net, imdb, getBatch, varargin)
...
if isempty(opts.train), opts.train = find(imdb.images.set(:,1)==1) ; end
...
assert(metaC{1}.id == 0);  % NOTE id==0 means missing metaclass label
Nmeta = numel(metaC);
metaC_idx = cell(1, Nmeta);

batchSize = params.batchSize;
assert(mod(batchSize, 2) == 0);
numBatches = zeros(Nmeta, 1);
for i = 1:Nmeta
    % randomize the groups (individual classes) within this metaclass
    G = metaC{i}.groups;
    G = G(randperm(numel(G)));
    metaC_idx{i} = cat(1, G{:});
    if i > 1
        numBatches(i) = ceil(2*metaC{i}.num/batchSize);
    end
end
logInfo('%d meta-classes, %d batches', Nmeta-1, sum(numBatches));
...
cur  = zeros(1, Nmeta);
t    = 0;
for m = 2:Nmeta, for p = 1:numBatches(m)
    % get half from metaclass #m, half from metaclass #1 (id:0)
    I1 = cur(1) + (1:batchSize/2);
    I2 = cur(m) + (1:batchSize/2);
    I1 = mod(I1-1, metaC{1}.num) + 1;
    I2 = mod(I2-1, metaC{m}.num) + 1;
    cur(1) = I1(end);
    cur(m) = I2(end);
    batch = [metaC_idx{1}(I1); metaC_idx{m}(I2)];
    t = t + batchSize;
    ...
    if toc(a) > 30
        fprintf('%s-%s ep%02d: %3d/%3d (%d) %.1fHz',  ...
            ...
            fix(t/batchSize), sum(numBatches), batchSize, averageSpeed) ;
...
for p=1:numel(net.params)
    ...
    case 'gradient'
        ...
        catch, net.params(p), keyboard, end
